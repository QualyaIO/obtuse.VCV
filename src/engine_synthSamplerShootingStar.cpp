...
void synthSamplerShootingStar_Processor_setNote(synthSamplerShootingStar_Processor__ctx_type_2 &_ctx, fix16_t gate, fix16_t voct, fix16_t vel, fix16_t retrigger, int cable){
   ...
   if(bool_not(_ctx.last_gates[cable]) && (gate >= 0x1999 /* 0.100000 */)){
      ...
      _ctx.last_pitches[cable] = synthSamplerShootingStar_Processor_cvToPitch(voct);
      _ctx.last_retrigger[cable] = true;
      synthSamplerShootingStar_Voice_noteOn(_ctx.voice,_ctx.last_pitches[cable],fix_to_int(velocity),0);
   }
   else
   {
      if(_ctx.last_gates[cable] && (gate < 0x1999 /* 0.100000 */)){
         if(_ctx.last_pitches[cable] >= 0){
            synthSamplerShootingStar_Voice_noteOff(_ctx.voice,_ctx.last_pitches[cable],0);
         }
   ...
   if(nbcables != _ctx.last_nbcables){
      ...
      while((c < 16) && (c < 16)){
         if(_ctx.last_gates[c] && (_ctx.last_pitches[c] >= 0)){
            synthSamplerShootingStar_Voice_noteOff(_ctx.voice,_ctx.last_pitches[c],0);
         }
